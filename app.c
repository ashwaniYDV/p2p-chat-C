...
void sendMessage(char* buff, char* identity, User users[], int userId, bool isBroadCast) {
	...
	peer_port = users[userId].port;
	strcpy(hostname, users[userId].ip);
...
// Send message to person mentioned
void sendOrBroadcastMessage(char* buff, char* identity, User users[], int num_users) {
	char** detail = parseMessage(buff);

	// (CHECK) Broadcast message to all
	if (strcmp(detail[0], "all") == 0) {
		for (int i = 0; i < num_users; i++) {
			sendMessage(buff, identity, users, i, true);
		}
		return;
	}
	...
	sendMessage(buff, identity, users, userId, false);
	return;
}
	...
	while (true) {
		...
		if (result == 0 && exit_flag) {
			...
			if ( FD_ISSET(STDIN, &readfds) ) {
				...
				sendOrBroadcastMessage(buff, identity, users, num_users);
				continue;
			}
